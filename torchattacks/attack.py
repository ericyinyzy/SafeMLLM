...
class Attack(object):
    ...
    def get_logits(self, inputs, labels=None, *args, **kwargs):
        ...
        # print('istrue',isinstance(inputs,list))
        if isinstance(inputs,list):
            # if self._normalization_applied is False:
            #     inputs[0] = self.normalize(inputs[0])
            logits = self.model(inputs)
        elif isinstance(inputs,dict):
            # print('caonimale',self._normalization_applied)
            if self._normalization_applied is False:
                if 'image' in inputs.keys():
                    inputs['image'] = self.normalize(inputs['image'])
                    logits = self.model(inputs)
                elif 'images' in inputs.keys():
                    inputs['images'] = self.normalize(inputs['images'])
                    # print(inputs['images'].dtype)
                    # exit()
                    logits = self.model(**inputs)
                elif  'pixel_values' in inputs.keys():
                    inputs['pixel_values'] = self.normalize(inputs['pixel_values'])
                    # print(inputs['pixel_values'].dtype,inputs['pixel_values'].shape,)
                    # exit()
                    logits = self.model(**inputs)

                else:
                    raise NotImplementedError('we only support attack on LLaVA, InstructBlip, Minigpt4 and internvl')
            logits = self.model(**inputs)
    ...
    def __call__(self, inputs, labels=None, *args, **kwargs):
        ...
        # print('hererrrrrr')
        print('here',self._normalization_applied)
