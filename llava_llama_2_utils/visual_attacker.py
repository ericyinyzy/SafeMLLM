...
class Attacker:
    ...
    def attack_unconstrained(self, text_prompt, img, batch_size = 8, num_iter=2000, alpha=1/255):
        ...
        # exit()
        sort_list = [i for i in range(len(text_prompt))]
        prompt = prompt_wrapper.Prompt( self.model, self.tokenizer, text_prompts=text_prompt, device=self.device )
        print(len(self.targets),len(text_prompt))
        for t in tqdm(range(num_iter + 1)):
            sort_idx = [int(t%25)]#%random.sample(sort_list, batch_size)

            # batch_targets = random.sample(self.targets, batch_size)
            batch_targets = [self.targets[i] for i in sort_idx]
            x_adv = normalize(adv_noise)

            target_loss = self.attack_loss(prompt, x_adv, batch_targets, sort_idx)
    ...
    #
    #     return loss
    def attack_loss(self, prompts, images, targets,sort_idx):

        context_length = [prompts.context_length[i] for i in sort_idx]
        context_input_ids = [prompts.input_ids[i] for i in sort_idx]
        batch_size = len(targets)
        # print(context_length,batch_size)
        # exit()
        ...
        #     targets[idx]=i+str(idx)
        # print(targets)
        # print(self.tokenizer(targets).input_ids)

        # exit()

        to_regress_tokens = [ torch.as_tensor([item[1:]]).cuda() for item in self.tokenizer(targets).input_ids] # get rid of the default <bos> in targets tokenization.
        # print(len(to_regress_tokens),len(context_input_ids))
        # exit()
        seq_tokens_length = []
        labels = []
        input_ids = []
        ...
        for i, item in enumerate(to_regress_tokens):
            ...
            L = item.shape[1] + context_length[i]
            # print(item.shape[1],context_length[i])
            seq_tokens_length.append(L)
            ...
            labels.append( torch.cat( [context_mask, item], dim=1 ) )
            input_ids.append( torch.cat( [context_input_ids[i], item], dim=1 ) )
            # print('11',i,context_input_ids[i].shape,item.shape,torch.cat( [context_input_ids[i], item], dim=1 ).shape)
            start_id_list.append(context_input_ids[i].shape[1]+254)
        ...
        # print(seq_tokens_length)
        max_length = max(seq_tokens_length)
        # print('maxlength',max_length,seq_tokens_length)
        # exit()
        attention_mask = []
        ...
        for i in range(batch_size):
            ...
            input_ids[i] = torch.cat( [input_ids[i],
                                       pad.repeat(1, num_to_pad)], dim=1 )
            # print('22',i,input_ids[i].shape,num_to_pad,pad.repeat(1, num_to_pad).shape)
            attention_mask.append( torch.LongTensor( [ [1]* (seq_tokens_length[i]) + [0]*num_to_pad ] ) )
            # print('newtimes', i)
