"""
SafeMLLM ImgJP attack on LLaVA-1.5-7B.

Generates a universal adversarial image (PGD-UAP) that, when paired with any of the
held-out AdvBench harmful prompts, makes the model produce a harmful answer. The
resulting tensor is saved to --advpath; per-prompt responses on the test split are
written to result_L7B/test_<advpath_stem>.csv.
"""
# Make the project root importable regardless of CWD (the script lives in attacks/).
import os, sys
...
if args.attack:
    ...
    test_csv = []
    print('statr test', model.test_num)
    ...
    os.makedirs('result_L7B', exist_ok=True)
    with open(os.path.join(f'result_L7B/test_{method}.csv'), 'w', encoding='utf-8', newline='') as f:
        write = csv.writer(f)
        rr = 0
